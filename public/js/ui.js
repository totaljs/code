	...
	self.configure = function(key, value, init) {
		...
		switch (key) {
			...
			case 'mode':
				...
				editor.setOption('lint', value === 'javascript' || value === 'xml' || value === 'totaljs' || value === 'totaljs_server' || value === 'html' ? { esversion: 11, expr: true, evil: true, unused: true, shadow: true, node: true, browser: true } : false);
...
COMPONENT('faicons', 'search:Search;scrollbarshadow:0', function(self, config, cls) {
...
COMPONENT('flowstream', 'left:0;top:0;zindex:100;language:eu', function(self, config, cls) {
	...
	self.iframe = function() {
		iframe && self.find('iframe').remove();
		self.append('<iframe src="{0}?hideclose=0&language={1}&darkmode={2}" scrolling="no" frameborder="0" allow="geolocation *; microphone *; camera *; midi *; encrypted-media *"></iframe>'.format(config.url, config.language, $('body').hclass('td') ? 1 : 0));
		iframe = self.find('iframe')[0];
		self.resize();
		self.rclass('hidden');
	};
